  ...
  return (
    <div style={{ padding: '28px 32px', maxWidth: 1000 }}>
      ...
      <div style={{ display: 'flex', gap: 10, marginBottom: 20 }}>
        {[0,1,2,3].map(i => (
          <div key={i} className="clay-raised" style={{ flex:1, padding: '12px 14px' }}>
      ...
      <div className="clay-raised" style={{ overflow: 'hidden' }}>
        <div style={{ padding: '10px 14px', boxShadow: 'inset 0 -1px 0 rgba(163,177,198,.3)', display: 'flex', gap: 12 }}>
          {[50,120,90,80,100,80,60].map((w, i) => (
            <Bone key={i} width={w} height={10} borderRadius={4} />
          ))}
        </div>
        {[0,1,2,3,4,5,7,8].map(i => (
          <div key={i} style={{ display: 'flex', gap: 12, padding: '11px 14px', boxShadow: 'inset 0 -1px 0 rgba(163,177,198,.18)' }}>
