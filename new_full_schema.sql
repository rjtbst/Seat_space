...
CREATE OR REPLACE FUNCTION "public"."confirm_subscription_payment_captured"("p_subscription_id" "uuid", "p_razorpay_order_id" "text", "p_razorpay_payment_id" "text", "p_actor_type" "text" DEFAULT 'reconciliation_cron'::"text", "p_actor_id" "uuid" DEFAULT NULL::"uuid", "p_webhook_event_id" "uuid" DEFAULT NULL::"uuid") RETURNS "jsonb"
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'public'
    AS $$
DECLARE
...
BEGIN
  ...
  IF v_sub.status = 'active' THEN
    RETURN jsonb_build_object('success', true, 'subscription_id', v_sub.id, 'already_confirmed', true);
  END IF;

  IF v_sub.status <> 'pending' THEN
    RETURN jsonb_build_object('success', false, 'error', format('subscription_status_%s', v_sub.status));
  END IF;

  SELECT * INTO v_payment
  FROM public.payments
  WHERE subscription_id = p_subscription_id AND razorpay_order_id = p_razorpay_order_id
  FOR UPDATE;
  ...
  IF v_payment.status = 'pending' THEN
    v_owner_payout := v_payment.base_amount;
    v_commission   := CASE WHEN v_payment.base_amount IS NOT NULL THEN v_payment.amount - v_payment.base_amount ELSE NULL END;

    UPDATE public.payments
    SET status                      = 'paid',
        razorpay_payment_id         = p_razorpay_payment_id,
        escrow_status                = 'not_applicable',
        owner_payout_amount         = v_owner_payout,
        platform_commission_amount = v_commission
    WHERE id = v_payment.id AND status = 'pending';
  END IF;

  UPDATE public.subscriptions SET status = 'active' WHERE id = v_sub.id AND status = 'pending';

  PERFORM public.log_financial_event(
    'payment', v_payment.id, 'captured', v_payment.amount,
    jsonb_build_object('payment_status', v_payment.status, 'subscription_status', v_sub.status),
    jsonb_build_object('payment_status', 'paid', 'subscription_status', 'active'),
    p_actor_type, p_actor_id, p_webhook_event_id,
    jsonb_build_object('razorpay_payment_id', p_razorpay_payment_id, 'razorpay_order_id', p_razorpay_order_id, 'subscription_id', v_sub.id)
  );

  RETURN jsonb_build_object('success', true, 'subscription_id', v_sub.id);
END;
$$;


ALTER FUNCTION "public"."confirm_subscription_payment_captured"("p_subscription_id" "uuid", "p_razorpay_order_id" "text", "p_razorpay_payment_id" "text", "p_actor_type" "text", "p_actor_id" "uuid", "p_webhook_event_id" "uuid") OWNER TO "postgres";
...
BEGIN
  ...
  -- Same overlap rule as prevent_booking_conflicting_with_subscription,
  -- checked here first purely to return a clean, specific error instead of
  -- a raw exclusion-violation from the trigger.
  SELECT s.id INTO v_conflict_id
  FROM public.subscriptions s
  JOIN public.plans p ON p.id = s.plan_id
  WHERE s.seat_id = p_seat_id
    AND s.status IN ('pending', 'active')
    AND s.start_date <= p_end_time
    AND s.end_date   >= p_start_time
    AND (
      p.time_window_start IS NULL
      OR (p.time_window_start < p_end_time::time AND p_start_time::time < p.time_window_end)
    )
    AND (
      p.days_of_week IS NULL
      OR (
        EXTRACT(DOW FROM p_start_time)::smallint = ANY(p.days_of_week)
        AND EXTRACT(DOW FROM p_end_time)::smallint = ANY(p.days_of_week)
      )
    )
  LIMIT 1;
...
CREATE OR REPLACE FUNCTION "public"."create_pending_subscription_with_payment"("p_user_id" "uuid", "p_plan_id" "uuid", "p_library_id" "uuid", "p_seat_id" "uuid", "p_razorpay_order_id" "text", "p_expected_total" numeric, "p_start_date" "date" DEFAULT NULL::"date", "p_coupon_code" "text" DEFAULT NULL::"text", "p_commission_bps" integer DEFAULT 500) RETURNS "jsonb"
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'public'
    AS $$
DECLARE
  ...
  v_conflict_id       uuid;
  v_stale_cutoff      constant interval := interval '20 minutes';
BEGIN
  IF auth.uid() IS DISTINCT FROM p_user_id THEN
    RETURN jsonb_build_object('success', false, 'error', 'not_authenticated_as_owner');
  END IF;
  ...
  IF NOT FOUND OR v_seat.library_id <> p_library_id THEN
    RETURN jsonb_build_object('success', false, 'error', 'seat_not_found');
  END IF;
  IF NOT v_seat.is_active THEN
    RETURN jsonb_build_object('success', false, 'error', 'seat_inactive');
  END IF;
  ...
  v_start_ts := COALESCE(p_start_date::timestamp, (now() AT TIME ZONE 'Asia/Kolkata')::date::timestamp);

  IF v_start_ts::date < ((now() AT TIME ZONE 'Asia/Kolkata')::date) THEN
    RETURN jsonb_build_object('success', false, 'error', 'start_date_in_past');
  END IF;

  v_end_ts   := v_start_ts + make_interval(days => COALESCE(v_plan.duration_days, 30)) + interval '23 hours 59 minutes 59 seconds';

  -- Release this exact seat's stale pending subscriptions (payment never
  -- completed, older than the cutoff) before checking for a conflict — an
  -- abandoned checkout must not permanently block the seat for everyone
  -- else. Fails the matching pending payment row first, same ordering
  -- used everywhere else in this schema for that reason.
  UPDATE public.payments pay
     SET status = 'failed'
    FROM public.subscriptions sub
   WHERE pay.subscription_id = sub.id
     AND sub.seat_id = p_seat_id
     AND sub.status = 'pending'
     AND sub.created_at < now() - v_stale_cutoff
  ...
  UPDATE public.subscriptions
   ...
   WHERE seat_id = p_seat_id
     AND status = 'pending'
     AND created_at < now() - v_stale_cutoff;

  -- Conflict against other subscriptions (pending/active) on this seat.
  SELECT s.id INTO v_conflict_id
  FROM public.subscriptions s
  JOIN public.plans p ON p.id = s.plan_id
  WHERE s.seat_id = p_seat_id
    AND s.status IN ('pending', 'active')
    AND s.start_date <= v_end_ts
    AND s.end_date   >= v_start_ts
    AND (
      p.time_window_start IS NULL OR v_plan.time_window_start IS NULL
      OR (p.time_window_start < v_plan.time_window_end AND v_plan.time_window_start < p.time_window_end)
    )
    AND (
      p.days_of_week IS NULL OR v_plan.days_of_week IS NULL
      OR p.days_of_week && v_plan.days_of_week
    )
  LIMIT 1;

  IF v_conflict_id IS NOT NULL THEN
    RETURN jsonb_build_object('success', false, 'error', 'seat_unavailable');
  END IF;

  -- Conflict against EXISTING ad-hoc bookings already sitting on this seat
  -- during hours the plan would reserve, anywhere in the subscription's
  -- date range — the mirror image of the bookings-side trigger, checked
  -- here so a subscription purchase can never silently kick out (or
  -- collide with) a booking that already exists.
  IF EXISTS (
    SELECT 1 FROM public.bookings b
    WHERE b.seat_id = p_seat_id
      AND b.status IN ('confirmed', 'held', 'checked_in')
      AND b.start_time <= v_end_ts
      AND b.end_time   >= v_start_ts
      AND (
        v_plan.time_window_start IS NULL
        OR (v_plan.time_window_start < b.end_time::time AND b.start_time::time < v_plan.time_window_end)
      )
      AND (
        v_plan.days_of_week IS NULL
        OR (
          EXTRACT(DOW FROM b.start_time)::smallint = ANY(v_plan.days_of_week)
          AND EXTRACT(DOW FROM b.end_time)::smallint = ANY(v_plan.days_of_week)
        )
      )
  ) THEN
    RETURN jsonb_build_object('success', false, 'error', 'seat_has_active_bookings');
  END IF;
  ...
  BEGIN
    INSERT INTO public.subscriptions (user_id, plan_id, library_id, seat_id, start_date, end_date, status)
    VALUES (p_user_id, p_plan_id, p_library_id, p_seat_id, v_start_ts, v_end_ts, 'pending')
...
ALTER FUNCTION "public"."create_pending_subscription_with_payment"("p_user_id" "uuid", "p_plan_id" "uuid", "p_library_id" "uuid", "p_seat_id" "uuid", "p_razorpay_order_id" "text", "p_expected_total" numeric, "p_start_date" "date", "p_coupon_code" "text", "p_commission_bps" integer) OWNER TO "postgres";
...
CREATE OR REPLACE FUNCTION "public"."find_stuck_pending_payments"("p_older_than" interval DEFAULT '00:30:00'::interval) RETURNS TABLE("payment_id" "uuid", "booking_id" "uuid", "subscription_id" "uuid", "razorpay_order_id" "text", "amount" numeric, "created_at" timestamp without time zone, "booking_status" "public"."booking_status", "subscription_status" "public"."subscription_status")
...
BEGIN
  ...
  RETURN QUERY
  SELECT p.id, p.booking_id, p.subscription_id, p.razorpay_order_id, p.amount, p.created_at, b.status, s.status
...
CREATE OR REPLACE FUNCTION "public"."prevent_booking_conflicting_with_subscription"() RETURNS "trigger"
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'public'
    AS $$
DECLARE
  v_conflict_id uuid;
BEGIN
  IF NEW.status NOT IN ('confirmed', 'held', 'checked_in') THEN
    RETURN NEW;
  END IF;
  ...
  WHERE s.seat_id = NEW.seat_id
    AND s.status IN ('pending', 'active')
    AND s.start_date <= NEW.end_time
    AND s.end_date   >= NEW.start_time
    AND (
      p.time_window_start IS NULL
      OR (p.time_window_start < NEW.end_time::time AND NEW.start_time::time < p.time_window_end)
    )
    AND (
      p.days_of_week IS NULL
      OR (
        EXTRACT(DOW FROM NEW.start_time)::smallint = ANY(p.days_of_week)
        AND EXTRACT(DOW FROM NEW.end_time)::smallint = ANY(p.days_of_week)
      )
    )
  LIMIT 1;

  IF v_conflict_id IS NOT NULL THEN
    RAISE EXCEPTION 'SEAT_RESERVED_BY_SUBSCRIPTION: seat % is reserved by subscription %', NEW.seat_id, v_conflict_id
...
ALTER FUNCTION "public"."prevent_booking_conflicting_with_subscription"() OWNER TO "postgres";


COMMENT ON FUNCTION "public"."prevent_booking_conflicting_with_subscription"() IS 'Backstop against ANY booking (student self-service, staff walk-in, owner walk-in, admin tools, or any future path) landing on a seat/time an active or pending subscription has reserved. Fires regardless of code path since it lives on the table itself, not any one RPC.';



CREATE OR REPLACE FUNCTION "public"."prevent_overlapping_seat_subscription"() RETURNS "trigger"
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'public'
    AS $$
DECLARE
  ...
  v_conflict_id   uuid;
BEGIN
  IF NEW.seat_id IS NULL OR NEW.status NOT IN ('pending', 'active') THEN
    RETURN NEW;
  END IF;
  ...
  WHERE s.seat_id = NEW.seat_id
    ...
    -- time windows overlap (NULL window = all day, always overlaps)
    AND (
      p.time_window_start IS NULL OR v_plan.time_window_start IS NULL
      OR (p.time_window_start < v_plan.time_window_end AND v_plan.time_window_start < p.time_window_end)
    )
    -- days of week overlap (NULL = every day, always overlaps)
    AND (
      p.days_of_week IS NULL OR v_plan.days_of_week IS NULL
      OR p.days_of_week && v_plan.days_of_week
    )
  ...
  IF v_conflict_id IS NOT NULL THEN
    RAISE EXCEPTION 'SEAT_ALREADY_RESERVED: seat % overlaps with subscription %', NEW.seat_id, v_conflict_id
      USING ERRCODE = 'exclusion_violation';
  END IF;
...
ALTER FUNCTION "public"."prevent_overlapping_seat_subscription"() OWNER TO "postgres";
...
CREATE OR REPLACE FUNCTION "public"."record_subscription_scan"("p_subscription_id" "uuid", "p_scanning_library_id" "uuid", "p_actor_id" "uuid") RETURNS "jsonb"
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'public'
    AS $$
DECLARE
  v_sub          public.subscriptions%ROWTYPE;
  v_plan         public.plans%ROWTYPE;
  v_open_visit   public.subscription_attendance%ROWTYPE;
  v_now          timestamp without time zone := (now() AT TIME ZONE 'Asia/Kolkata');
  v_attendance_id uuid;
BEGIN
  IF NOT (
    EXISTS (SELECT 1 FROM public.libraries WHERE id = p_scanning_library_id AND owner_id = auth.uid())
    OR public.is_staff_of(p_scanning_library_id)
  ) THEN
    RETURN jsonb_build_object('success', false, 'error', 'not_authorized');
  END IF;
  ...
  IF v_sub.status <> 'active' THEN
    RETURN jsonb_build_object('success', false, 'error', 'subscription_not_active');
  END IF;
  IF v_sub.library_id IS DISTINCT FROM p_scanning_library_id THEN
    RETURN jsonb_build_object('success', false, 'error', 'wrong_library');
  END IF;
  IF v_now < v_sub.start_date OR v_now > v_sub.end_date THEN
  ...
  IF v_plan.time_window_start IS NOT NULL THEN
    IF v_now::time < v_plan.time_window_start OR v_now::time > v_plan.time_window_end THEN
      RETURN jsonb_build_object(
  ...
  IF v_plan.days_of_week IS NOT NULL AND NOT (EXTRACT(DOW FROM v_now)::smallint = ANY(v_plan.days_of_week)) THEN
    RETURN jsonb_build_object('success', false, 'error', 'invalid_day', 'days_of_week', v_plan.days_of_week);
  END IF;

  -- Already-open visit today (checked in, not yet checked out) -> this
  -- scan is the check-out.
  SELECT * INTO v_open_visit
  FROM public.subscription_attendance
  WHERE subscription_id = p_subscription_id
    AND check_out_time IS NULL
    AND check_in_time::date = v_now::date
  ORDER BY check_in_time DESC
  LIMIT 1;

  IF FOUND THEN
    UPDATE public.subscription_attendance
    SET check_out_time = v_now, checked_out_by = p_actor_id
    WHERE id = v_open_visit.id;

    RETURN jsonb_build_object('success', true, 'action', 'checked_out', 'attendance_id', v_open_visit.id);
  END IF;

  INSERT INTO public.subscription_attendance (subscription_id, library_id, seat_id, check_in_time, checked_in_by)
  VALUES (p_subscription_id, p_scanning_library_id, v_sub.seat_id, v_now, p_actor_id)
  RETURNING id INTO v_attendance_id;

  RETURN jsonb_build_object('success', true, 'action', 'checked_in', 'attendance_id', v_attendance_id);
END;
$$;


ALTER FUNCTION "public"."record_subscription_scan"("p_subscription_id" "uuid", "p_scanning_library_id" "uuid", "p_actor_id" "uuid") OWNER TO "postgres";
...
CREATE OR REPLACE FUNCTION "public"."sweep_expire_stale_pending_subscriptions"() RETURNS integer
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'public'
    AS $$
DECLARE
  v_cutoff constant interval := interval '20 minutes';
  v_count  integer;
BEGIN
  UPDATE public.payments pay
     SET status = 'failed'
   ...
   WHERE pay.subscription_id = sub.id
     AND sub.status = 'pending'
     AND sub.created_at < now() - v_cutoff
     AND pay.status = 'pending';
...
ALTER FUNCTION "public"."sweep_expire_stale_pending_subscriptions"() OWNER TO "postgres";


COMMENT ON FUNCTION "public"."sweep_expire_stale_pending_subscriptions"() IS 'Cancels pending subscriptions (and fails their pending payment row) whose checkout was abandoned more than 20 minutes ago, freeing the seat they were holding. Mirrors sweep_expire_stale_holds for bookings. Schedule via pg_cron alongside it; also called from /api/cron/reconcile-payments as a second trigger.';
...
CREATE OR REPLACE FUNCTION "public"."sync_library_into_cross_scope_plans"() RETURNS "trigger"
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'public'
    AS $$
BEGIN
  IF NEW.is_active THEN
    -- New or reactivated library: link it to every one of this owner's
    -- existing "all my libraries" plans. ON CONFLICT is a no-op for the
    -- INSERT-time firing (nothing to conflict with yet) and makes the
    -- is_active-toggle-to-true firing idempotent if it somehow already
    -- has the row.
    INSERT INTO public.plan_libraries (plan_id, library_id)
    SELECT p.id, NEW.id
    FROM public.plans p
    WHERE p.owner_id = NEW.owner_id
      AND p.scope = 'cross'
    ON CONFLICT (plan_id, library_id) DO NOTHING;
  ELSE
    -- Deactivated library: drop it from cross-scope plans only. Plans
    -- where the owner manually, deliberately picked this specific library
    -- (scope = 'library') are left untouched — that was an intentional
    -- choice, not something this auto-sync should silently undo, and
    -- restores correctly if the library is reactivated (branch re-links
    -- via the IF branch above).
    DELETE FROM public.plan_libraries pl
    USING public.plans p
    WHERE pl.plan_id = p.id
      AND p.scope = 'cross'
      AND pl.library_id = NEW.id;
  END IF;
...
ALTER FUNCTION "public"."sync_library_into_cross_scope_plans"() OWNER TO "postgres";


COMMENT ON FUNCTION "public"."sync_library_into_cross_scope_plans"() IS 'Keeps plan_libraries in sync with an owner''s library list for scope=cross ("all my libraries") plans -- fires on every library insert/reactivate/deactivate, regardless of which code path did it, so a cross-scope plan can never silently go stale.';
...
ALTER TABLE "public"."subscription_attendance" OWNER TO "postgres";


COMMENT ON TABLE "public"."subscription_attendance" IS 'One row per physical visit by a subscribed student, created when staff/owner scans the student''s subscription QR code. Independent of `bookings` — subscribed students do not create a daily booking row.';
...
COMMENT ON COLUMN "public"."subscriptions"."library_id" IS 'The single library this subscription is attached to (chosen by the student at purchase time from the plan''s applicable libraries). NULL only for legacy pre-migration rows.';



COMMENT ON COLUMN "public"."subscriptions"."seat_id" IS 'The seat reserved for this subscription''s entire duration, within the plan''s time window/days. Enforced unique-per-active-window by prevent_overlapping_seat_subscription(). NULL only for legacy pre-migration rows.';
...
ALTER TABLE ONLY "public"."subscription_attendance"
    ADD CONSTRAINT "subscription_attendance_pkey" PRIMARY KEY ("id");
...
CREATE INDEX "idx_subscription_attendance_library_date" ON "public"."subscription_attendance" USING "btree" ("library_id", "check_in_time" DESC);



CREATE INDEX "idx_subscription_attendance_subscription" ON "public"."subscription_attendance" USING "btree" ("subscription_id", "check_in_time" DESC);



CREATE INDEX "idx_subscriptions_library" ON "public"."subscriptions" USING "btree" ("library_id");
...
CREATE INDEX "idx_subscriptions_seat_active" ON "public"."subscriptions" USING "btree" ("seat_id", "status") WHERE ("status" = ANY (ARRAY['pending'::"public"."subscription_status", 'active'::"public"."subscription_status"]));
...
CREATE OR REPLACE TRIGGER "trg_prevent_booking_conflicting_with_subscription" BEFORE INSERT OR UPDATE OF "seat_id", "start_time", "end_time", "status" ON "public"."bookings" FOR EACH ROW EXECUTE FUNCTION "public"."prevent_booking_conflicting_with_subscription"();



CREATE OR REPLACE TRIGGER "trg_prevent_overlapping_seat_subscription" BEFORE INSERT OR UPDATE OF "seat_id", "status", "start_date", "end_date" ON "public"."subscriptions" FOR EACH ROW EXECUTE FUNCTION "public"."prevent_overlapping_seat_subscription"();
...
CREATE OR REPLACE TRIGGER "trg_sync_library_into_cross_scope_plans" AFTER INSERT OR UPDATE OF "is_active" ON "public"."libraries" FOR EACH ROW EXECUTE FUNCTION "public"."sync_library_into_cross_scope_plans"();
...
ALTER TABLE ONLY "public"."subscription_attendance"
    ADD CONSTRAINT "subscription_attendance_checked_in_by_fkey" FOREIGN KEY ("checked_in_by") REFERENCES "public"."users"("id");



ALTER TABLE ONLY "public"."subscription_attendance"
    ADD CONSTRAINT "subscription_attendance_checked_out_by_fkey" FOREIGN KEY ("checked_out_by") REFERENCES "public"."users"("id");



ALTER TABLE ONLY "public"."subscription_attendance"
    ADD CONSTRAINT "subscription_attendance_library_id_fkey" FOREIGN KEY ("library_id") REFERENCES "public"."libraries"("id");



ALTER TABLE ONLY "public"."subscription_attendance"
    ADD CONSTRAINT "subscription_attendance_seat_id_fkey" FOREIGN KEY ("seat_id") REFERENCES "public"."seats"("id");



ALTER TABLE ONLY "public"."subscription_attendance"
    ADD CONSTRAINT "subscription_attendance_subscription_id_fkey" FOREIGN KEY ("subscription_id") REFERENCES "public"."subscriptions"("id");



ALTER TABLE ONLY "public"."subscriptions"
    ADD CONSTRAINT "subscriptions_library_id_fkey" FOREIGN KEY ("library_id") REFERENCES "public"."libraries"("id");
...
ALTER TABLE ONLY "public"."subscriptions"
    ADD CONSTRAINT "subscriptions_seat_id_fkey" FOREIGN KEY ("seat_id") REFERENCES "public"."seats"("id");
...
CREATE POLICY "subscription_attendance_admin_select" ON "public"."subscription_attendance" FOR SELECT USING ("public"."is_admin"());



CREATE POLICY "subscription_attendance_owner_all" ON "public"."subscription_attendance" USING ((EXISTS ( SELECT 1
   FROM "public"."libraries" "l"
  WHERE (("l"."id" = "subscription_attendance"."library_id") AND ("l"."owner_id" = "auth"."uid"())))));



CREATE POLICY "subscription_attendance_staff_all" ON "public"."subscription_attendance" USING ("public"."is_staff_of"("library_id"));



CREATE POLICY "subscription_attendance_student_select" ON "public"."subscription_attendance" FOR SELECT USING ((EXISTS ( SELECT 1
   FROM "public"."subscriptions" "s"
  WHERE (("s"."id" = "subscription_attendance"."subscription_id") AND ("s"."user_id" = "auth"."uid"())))));
...
GRANT ALL ON FUNCTION "public"."confirm_subscription_payment_captured"("p_subscription_id" "uuid", "p_razorpay_order_id" "text", "p_razorpay_payment_id" "text", "p_actor_type" "text", "p_actor_id" "uuid", "p_webhook_event_id" "uuid") TO "anon";
GRANT ALL ON FUNCTION "public"."confirm_subscription_payment_captured"("p_subscription_id" "uuid", "p_razorpay_order_id" "text", "p_razorpay_payment_id" "text", "p_actor_type" "text", "p_actor_id" "uuid", "p_webhook_event_id" "uuid") TO "authenticated";
GRANT ALL ON FUNCTION "public"."confirm_subscription_payment_captured"("p_subscription_id" "uuid", "p_razorpay_order_id" "text", "p_razorpay_payment_id" "text", "p_actor_type" "text", "p_actor_id" "uuid", "p_webhook_event_id" "uuid") TO "service_role";
...
GRANT ALL ON FUNCTION "public"."create_pending_subscription_with_payment"("p_user_id" "uuid", "p_plan_id" "uuid", "p_library_id" "uuid", "p_seat_id" "uuid", "p_razorpay_order_id" "text", "p_expected_total" numeric, "p_start_date" "date", "p_coupon_code" "text", "p_commission_bps" integer) TO "anon";
GRANT ALL ON FUNCTION "public"."create_pending_subscription_with_payment"("p_user_id" "uuid", "p_plan_id" "uuid", "p_library_id" "uuid", "p_seat_id" "uuid", "p_razorpay_order_id" "text", "p_expected_total" numeric, "p_start_date" "date", "p_coupon_code" "text", "p_commission_bps" integer) TO "authenticated";
GRANT ALL ON FUNCTION "public"."create_pending_subscription_with_payment"("p_user_id" "uuid", "p_plan_id" "uuid", "p_library_id" "uuid", "p_seat_id" "uuid", "p_razorpay_order_id" "text", "p_expected_total" numeric, "p_start_date" "date", "p_coupon_code" "text", "p_commission_bps" integer) TO "service_role";
...
GRANT ALL ON FUNCTION "public"."prevent_booking_conflicting_with_subscription"() TO "anon";
GRANT ALL ON FUNCTION "public"."prevent_booking_conflicting_with_subscription"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."prevent_booking_conflicting_with_subscription"() TO "service_role";



GRANT ALL ON FUNCTION "public"."prevent_overlapping_seat_subscription"() TO "anon";
GRANT ALL ON FUNCTION "public"."prevent_overlapping_seat_subscription"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."prevent_overlapping_seat_subscription"() TO "service_role";
...
GRANT ALL ON FUNCTION "public"."record_subscription_scan"("p_subscription_id" "uuid", "p_scanning_library_id" "uuid", "p_actor_id" "uuid") TO "anon";
GRANT ALL ON FUNCTION "public"."record_subscription_scan"("p_subscription_id" "uuid", "p_scanning_library_id" "uuid", "p_actor_id" "uuid") TO "authenticated";
GRANT ALL ON FUNCTION "public"."record_subscription_scan"("p_subscription_id" "uuid", "p_scanning_library_id" "uuid", "p_actor_id" "uuid") TO "service_role";
...
GRANT ALL ON FUNCTION "public"."sweep_expire_stale_pending_subscriptions"() TO "anon";
GRANT ALL ON FUNCTION "public"."sweep_expire_stale_pending_subscriptions"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."sweep_expire_stale_pending_subscriptions"() TO "service_role";
...
GRANT ALL ON FUNCTION "public"."sync_library_into_cross_scope_plans"() TO "anon";
GRANT ALL ON FUNCTION "public"."sync_library_into_cross_scope_plans"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."sync_library_into_cross_scope_plans"() TO "service_role";
...
GRANT ALL ON TABLE "public"."subscription_attendance" TO "anon";
GRANT ALL ON TABLE "public"."subscription_attendance" TO "authenticated";
GRANT ALL ON TABLE "public"."subscription_attendance" TO "service_role";
