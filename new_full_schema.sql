...
COMMENT ON FUNCTION "public"."create_held_booking_with_payment"("p_user_id" "uuid", "p_library_id" "uuid", "p_seat_id" "uuid", "p_start_time" timestamp without time zone, "p_end_time" timestamp without time zone, "p_hold_expires_at" timestamp without time zone, "p_amount" numeric, "p_razorpay_order_id" "text", "p_base_amount" numeric) IS 'Atomically creates a held booking + its pending payment row. p_amount is the GROSS amount charged via Razorpay (library price + platform fee); p_base_amount is the library''s listed price alone, used at confirm-time to give the owner their payout without any rounding drift. If the payment insert fails for any reason, the booking insert rolls back too. Callable by authenticated users for their own user_id only.';
